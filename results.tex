...
For the hospital trial, the proposed \acs{IoT} system has been deployed in a clinical facility within Centro Hospitalar e Universitário de Coimbra (CHUC), during which two volunteers were continuously monitored using the system. A \textit{Smart box} and two \textit{Biostickers} are assigned to each volunteer for the duration of the trial. One of the \textit{Biostickers} recorded Pulse Oximeter data and was attached to the patient's right-hand index finger, and the other \textit{Biosticker} was attached to the patient's chest recording the other sensors (body temperature, \acs{ECG}, \dots, as discussed in Section \ref{sec:biosticker_data}). The patients remained in bed the entirety of the trial, always keeping the \textit{Smart box} at most 5 meters away from the patient. The Wi-Fi network used to connect the \textit{Smart box} to the \textit{Smart Gateway} was provided by the hospital IT, and was being used concurrently by the researchers during the study. Additionally, each device was assigned a fixed \acs{IP} to facilitate the deployment of the infrastructure, and ensure the devices were able to communicate with one another at all times.
...
In this trial, the objective is to analyze the stability and reliability of the infrastructure in a real-world scenario. To this end, the bandwidth used by the \acs{MQTT} and \acs{FHIR} communication protocols are used to evaluate reliability of the system, in order to check for interruptions of the communication and observe if there's some issue with the communication; the system resource usage (\acs{CPU} and \acs{RAM}) is also monitored to evaluate the system's stability. To evaluate the performance of the proposed system, the following performance metrics were defined and measured throughout the tests:
...
\paragraph{} Overall, the results reported are extremely positive, since they demonstrate the performance of the system developed throughout this past year. Certain phenomenons should be researched further, for example the observed latency deviations on \acs{MQTT} communications, but nonetheless, the developed architecture should provide a solid foundation for the ongoing development of the \acs{WoW} project.
