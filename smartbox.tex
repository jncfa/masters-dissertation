...
\todo[inline]{Todo: Confirm with Oxygen Saturation acquisition rate from the pilot data.}
...
% concordo, se quiseres, basta apagares o texto (por ex. no paint), mantendo as caixas e as setas, depois importar por ex. para dentro do powerpoint, colocar texto dentro das caixas e guardar a figura em PDF (vectorial) para a incluir aqui.
...
\paragraph{} We also notice that the roundtrip graphs for the ASUS USB-BT500 tests, in particular on Figure \ref{fig:ble-roundtrip-hci0-0m}, have a shape similar to that of a ``stepping function''. By analyzing the data from Figure \ref{fig:ble-roundtrip-hci0-0m}, we find that on average each step has a width of $36.2 \pm 0.98$ bytes, and a height difference of $9.18 \pm 1.18$ms (which is quite close to the connection interval, 7.5ms).

\paragraph{} This likely indicates that the data is being fragmented on the \acs{L2CAP} every $\sim$36 bytes (meaning it is the maximum payload size that can be sent in a single connection event), and split across multiple connection intervals, instead of being transmitted in a single packet. 
...
\paragraph{} Due to all the aforementioned reasons, we have decided to move forward with the ASUS USB-BT500 adapter for the development of \acs{BLE} data acquisition in the scope of the \acs{WoW} project.
