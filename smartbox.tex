...
{\color{blue} As discussed, there are multiple requirements the \textit{Smart box} hardware must fulfill, from communication to the \textit{Biostickers} to integration with the \textit{Smart Gateway}:}

{\color{blue}\begin{itemize}
    \item The device must be small enough to be mounted on a hospital bed.
    \item The device must be powered via a standard electrical outlet, so power consumption constraints are not an issue.
    \item It must support \acs{BLE} 5.0, and capable of handling the communication bandwidth required by the \textit{Biostickers}.
    \item The hardware must support Ubuntu 20.04 LTS as its operative system. 
    \item It must communicate to the \textit{Smart Gateway} using the protocol \acs{MQTT}, connected via Wi-Fi.
\end{itemize}
}
% - analise de requisitos: o q necessito? eficiencia do ponto de vista energético (baterias ou alimentação?), low consumption? quantidade se sensores? dados? suporta linux? payloads das msgs? - adicionar análise de requisitos 

{\color{blue}With these requirements in mind, and} in context of this dissertation, two different \acl{SBC}s (\acs{SBC}) have been considered for the development of the \textit{Smart box}: a Raspberry Pi 4 Model B and an UDOO BOLT v3. In the following sections the characteristics of each platform are discussed and compared. 
...
\begin{table}[H]
    ...
    \begin{tabular}{r|l|l}
        ...
        {\color{blue} \makecell[r]{\textbf{Max. Power} \\ \textbf{Consumption}}} & {\color{blue}9 W} & {\color{blue} 60 W}  \\
...
%\footnotetext{\url{https://www.raspberrypi.com/documentation/computers/raspberry-pi.html}}
%\footnotetext{\url{https://www.udoo.org/docs-bolt/Introduction/Introduction.html}}
