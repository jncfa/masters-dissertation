...
At the time of the writing, the Raspberry Pi 4 Model B (or Raspberry Pi 4B) \footnote{\url{https://www.raspberrypi.com/products/raspberry-pi-4-model-b/}} is the latest revision of the Raspberry Pi series, powered by Broadcom BCM2711 System on a Chip (SoC). 
...
The Host Layer contains the higher level protocols of the protocol stack, such as \acf{L2CAP}, \acf{GAP},  while the Controller Layer handles the lower layers related to 
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption[Average \acs{BLE} connection roundtrip time obtained for different distances using the internal \acs{BLE} adapter provided in the Raspberry Pi 4B.]{Average \acs{BLE} connection roundtrip time obtained for different distances using the internal \acs{BLE} adapter provided in the Raspberry Pi 4B.}
    \label{fig:ble-roundtrip-hci1}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption[Packet loss measured for different transmission frequencies using the internal \acs{BLE} adapter provided in the Raspberry Pi 4B.]{Average \acs{BLE} connection roundtrip time obtained for different distances using the internal \acs{BLE} adapter provided in the Raspberry Pi 4B.}
    \label{fig:ble-bandwidth-hci1}
\end{figure}
